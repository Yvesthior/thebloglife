...
class HookServiceProvider extends ServiceProvider
{
    ...
    public function addPublishFields()
    {
        ...
        $data = $args[0];
        if ($data && $data->id) {
            $publishDate = BaseHelper::formatDate($data->published_at);
            $publishTime = BaseHelper::formatDate($data->published_at, 'G:i');
        }
    ...
    public function saveSchedulerData($screen, $request, $object)
    {
        if (PostScheduler::isSupportedModule(get_class($object))) {
            ...
            if (!empty($publishDate)) {
                $object->published_at = Carbon::createFromFormat(config('core.base.general.date_format.date') . ' G:i', $publishDate . ' ' . $publishTime)->toDateTimeString();
                $object->save();
            }
        }
    }
    ...
    public function checkPublishDateBeforeShow($data, $model)
    {
        if (PostScheduler::isSupportedModule(get_class($model))) {
            ...
            $data->where(function ($query) use ($table) {
                return $query->where($table . '.published_at', '<=', now(config('app.timezone'))->toDateTimeString())->orWhereNull($table . '.published_at');
            });
        }
