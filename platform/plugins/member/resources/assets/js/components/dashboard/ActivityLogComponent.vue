            ...
            <div :class="[activeTab === activityLogTab ? 'tab-pane fade show active' : 'tab-pane fade']"
                 :id="activityLogTab">
                    ...
                    <div v-if="!isLoading && activityLogs.links.next" class="tc light-gray-text">
                        <a class="f6 mv2" href="javascript:void(0)" v-if="!isLoadingMore" @click="getActivityLogs(activityLogs.links.next)">{{ __('load_more') }}</a>
    ...
    export default {
        ...
        methods: {
            ...
            getActivityLogs(url = null) {
                ...
                axios.get(url ? url : '/ajax/members/activity-logs')
                    .then(res =>  {
                        let oldData = [];
